 ...
 */
@SuppressLint("NewApi")
public abstract class Chart<T extends ChartData<? extends DataSet<? extends Entry>>> extends ViewGroup
	...
	/**
	 * 设置linechart的x轴和y轴是否显示
	 */
	protected boolean xMarkers;

	protected boolean yMarkers;
	...
	public boolean valuesToHighlight() {
		return mIndicesToHightlight == null || mIndicesToHightlight.length <= 0 || mIndicesToHightlight[0] == null
				? false : true;
	}
	...
	protected MarkerView mMarkerViewX;
	protected MarkerView mMarkerViewY;

	/**
	 * draws all MarkerViews on the highlighted positions
	 */
	/**
	 * 设置当前linechartx轴和y轴是否显示
	 * 
	 * @param xMarkers
	 * @param yMarkers
	 */
	protected void setMarkers(boolean xMarkers, boolean yMarkers) {
	...
	protected void drawMarkers(Canvas canvas) {
		...
		if (xMarkers) {
			...
			if (mMarkerViewX == null || !mDrawMarkerViews || !valuesToHighlight())
				return;
			...
			for (int i = 0; i < mIndicesToHightlight.length; i++) {
				...
				if (xIndex <= mDeltaX && xIndex <= mDeltaX * mAnimator.getPhaseX()) {
					...
					mMarkerViewX.refreshContent(e, dataSetIndex);
					...
					mMarkerViewX.measure(MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED),
							MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED));
					mMarkerViewX.layout(0, 0, mMarkerViewX.getMeasuredWidth(), mMarkerViewX.getMeasuredHeight());

					// Log.i("chart",
					// "getwidth="+getWidth()+"pos[0]="+pos[0]+"pos[1]="+pos[1]+"markerview="+mMarkerViewX.getWidth());
					// if (getWidth()-pos[0]<=mMarkerViewX.getWidth()) {
					// Toast.makeText(getContext(), "�߽�",
					// Toast.LENGTH_SHORT).show();
					// }

					if (canvas.getWidth() - pos[0] < mMarkerViewX.getWidth() / 2) {
						Log.i("-----", "�ұ߽�");

						mMarkerViewX.draw(canvas, canvas.getWidth() - mMarkerViewX.getWidth() / 2, getHeight());
					} else if (pos[0] < mMarkerViewX.getWidth() / 2) {
						Log.i("-----", "��߽�");
						mMarkerViewX.draw(canvas, mMarkerViewX.getWidth()/2, getHeight());
					} else {
						mMarkerViewX.draw(canvas, pos[0], getHeight());
					}

					Log.i("------", "pos[0]=" + pos[0] + "pos[1]=" + pos[1]);
					// if (pos[1] - mMarkerViewX.getHeight() <= 0) {
					// float y = mMarkerViewX.getHeight() - pos[1];
					// // mMarkerView.draw(canvas, pos[0], pos[1] + y);
					// mMarkerViewX.draw(canvas, pos[0], getHeight());
					// Log.i("------", "pos[0]=" + pos[0] + "y=" + y);
					//// Toast.makeText(getContext(), "pos[0]=" + pos[0] + "y="
					// + y, Toast.LENGTH_SHORT).show();
					// } else {
					// // mMarkerView.draw(canvas, pos[0], pos[1]);
					// mMarkerViewX.draw(canvas, pos[0], getHeight());
					// Log.i("------", "pos[0]=" + pos[0] + "pos[1]=" + pos[1]);
					//// Toast.makeText(getContext(), "pos[0]=" + pos[0] +
					// "pos[1]=" + pos[1], Toast.LENGTH_SHORT)
					//// .show();
					// }
				}
			}
		...
		if (yMarkers) {
			if (mMarkerViewY == null || !mDrawMarkerViews || !valuesToHighlight())
				return;
			...
			for (int i = 0; i < mIndicesToHightlight.length; i++) {
				...
				if (xIndex <= mDeltaX && xIndex <= mDeltaX * mAnimator.getPhaseX()) {
					...
					// callbacks to update the content
					mMarkerViewY.refreshContent(e, dataSetIndex);
					...
					mMarkerViewY.measure(MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED),
							MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED));
					mMarkerViewY.layout(0, 0, mMarkerViewX.getMeasuredWidth(), mMarkerViewY.getMeasuredHeight());

					if (pos[1] - mMarkerViewY.getHeight() <= 0) {
						float y = mMarkerViewY.getHeight() - pos[1];
						// mMarkerView.draw(canvas, pos[0], pos[1] + y);
						mMarkerViewY.draw(canvas, 0, pos[1]);
						// Toast.makeText(getContext(), "pos[0]=" + pos[0] +
	 ...
	 * @param v
	 */
	public void setMarkerViewX(MarkerView v) {
		mMarkerViewX = v;
	 ...
	 * @return
	 */
	public MarkerView getMarkerViewX() {
		return mMarkerViewX;
	}

	public void setMarkerViewY(MarkerView v) {
		mMarkerViewY = v;
	}

	public MarkerView getMarkerViewY() {
		return mMarkerViewY;
	}
